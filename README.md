...
Open **Features → Create a section** for a dedicated live section designer. Eight starters cover heroes, bento features, statistics, steps, testimonials, pricing, FAQs, and calls to action. Customize text, cards, links, colors, dark mode and corners; switch between desktop and mobile preview. Sample statistics, prices and testimonials need your real content before publishing.

Drafts stay in the current browser. **Save a copy to library** stores a private reusable section in the existing cloud feature library for signed-in users. Search, export/import, or edit saved copies without changing sections already installed in projects. Reuse previews the file changes and saves a new project version without an AI call. Browser projects append the section; Next.js projects insert a scoped component at the end of the chosen page’s single JSX `main` element. Ambiguous insertion points are rejected. Existing files, database schema, and runtime dependencies are preserved. The visual designer remains available alongside AI Feature Studio.

### AI Feature Studio

Open **Features → Build a feature with AI** to create custom sections and interactive browser features from a prompt. Follow-up requests include the current code and previous turn. The preview retains the previous version while generation runs. Name and save copies to the private library; **Refine with AI** reopens the saved code and conversation. Insert a saved feature through the source review flow at no further AI charge.

Features run in an isolated iframe with local scripts, a configurable 200–1400px height, and mobile preview. Network requests, form submissions and persistent storage are blocked; database-backed features require separate project integration. Generation uses the existing authenticated OpenAI configuration, routing, usage tracking and request deduplication, with no automatic paid retries. Save before closing to retain your work.
