...
test('routine chat edit checks limits and submits one stage without a planning dialog',async()=>{
 ...
 assert.equal(result.job.id,'job');assert.equal(calls.length,2);const plan=calls[1].body.plan;assert.equal(plan.stages.length,1);assert.equal(plan.baseRevision,'revision');assert.equal(plan.budgetUsd,2);assert.equal(plan.allowRepair,false);assert.deepEqual(calls[1].body.images,images);
});
test('budget block sends no build request and does not increase configured limits',async()=>{
 ...
 await assert.rejects(submitChatBuild(project,revision,'Use the logo',[],async()=>({maxBuildUsd:1,remainingMonthUsd:10,reservedUsd:2})),/spending limit/);
});
test('daily token preflight includes unresolved reservations, reports room needed and respects rolling expiry',()=>{
 ...
 assert.match(budgetBlock(usage,300,.1,now)!,/200 available.*300/);
