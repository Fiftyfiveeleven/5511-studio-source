...
test('saved logos remain visual context and a screenshot does not need to become the logo',async()=>{
 const {estimateGeneration,generateApp}=await import('../src/lib/generate');const {rememberedImages}=await import('../src/lib/image-assets');const {mkdtemp,rm}=await import('node:fs/promises');const {tmpdir}=await import('node:os');const dir=await mkdtemp(tmpdir()+'/followup-');const old=process.env.STUDIO_USAGE_DIR,fetch=globalThis.fetch;process.env.STUDIO_USAGE_DIR=dir;
 const files=attachAssets([{path:'index.html',content:'<h1>Brand</h1>'}],[image]);const screenshot={name:'Screenshot.png',dataUrl:'data:image/png;base64,iVBORw0KGgo=',useAs:'reference' as const};
 try{assert.equal(rememberedImages(files)[0].dataUrl,image.dataUrl);const estimate=await estimateGeneration('followup-test','Make the logo half the size','App',files,false,{images:[screenshot],previousTurn:{prompt:'Use the uploaded logo',summary:'Placed the logo in the header'}});assert.equal(estimate.images.length,2);assert.ok(estimate.context.includes('Placed the logo'));assert.ok(!estimate.context.includes(image.dataUrl));globalThis.fetch=async()=>Response.json({id:'r',object:'response',status:'completed',output:[{id:'m',type:'message',role:'assistant',status:'completed',content:[{type:'output_text',text:JSON.stringify({name:'App',summary:'Reduced header image size',files:[{path:'styles.css',content:'header img {max-height:40px;width:auto}'}],sql:null}),annotations:[]}]}],usage:{input_tokens:100,output_tokens:20,total_tokens:120}});const result=await generateApp('followup-test','Make the logo half the size','App',files,false,{images:[screenshot]});assert.equal(rememberedImages(result.files)[0].dataUrl,image.dataUrl);assert.ok(result.files.some(f=>f.path==='styles.css'));}finally{globalThis.fetch=fetch;if(old===undefined)delete process.env.STUDIO_USAGE_DIR;else process.env.STUDIO_USAGE_DIR=old;await rm(dir,{recursive:true,force:true})}
});
