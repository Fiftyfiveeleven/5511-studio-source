...
export async function POST(request:Request){try{requireSameOrigin(request);const {db,user}=await authorize(request);const input=z.object({id:z.uuid(),name:z.string().min(1).max(80),description:z.string().max(6000),url:z.string().nullable(),key:z.string().nullable(),revision:z.object({files:artifactSchema.shape.files,sql:z.string().max(50000),tokens:z.number().int().min(0).max(10000000)}).nullable()}).parse(await readBody(request));
