import {reconcileProjectJobs,enqueueError} from '@/lib/job-recovery';
import {z} from 'zod';
...
export async function GET(request:Request,ctx:Context){try{const {db}=await authorize(request);const {id}=await ctx.params;await ownedProject(db,id);await reconcileProjectJobs(id);const {data,error}=await db.from('studio_build_jobs').select('id,status,stage,progress,plan,runtime,expected_revision,report,error,cancel_requested,created_at,updated_at').eq('project_id',id).neq('plan->stages','[]').order('created_at',{ascending:false}).limit(5);if(error)throw new HttpError(503,'Background job storage needs the Phase 1 migration.');return Response.json({jobs:data},{headers:{'Cache-Control':'no-store'}})}catch(e){return failure(e)}}
export async function POST(request:Request,ctx:Context){try{requireSameOrigin(request);const {db,user}=await authorize(request);const {id}=await ctx.params;const p=await ownedProject(db,id);await requireEditor(db,id,user.id);const input=z.object({plan:buildPlanSchema,images:attachmentsSchema,runtime:z.enum(['browser','nextjs'])}).parse(await readBody(request,2000000));if(!runtimeReady())throw new HttpError(503,'Enable Vercel Sandbox in Studio server settings before background builds.');const key=credential(request,'openai')||process.env.OPENAI_API_KEY;if(!key)throw new HttpError(400,'Connect OpenAI first.');if(input.plan.baseRevision!==p.current_revision_id)throw new HttpError(409,'Project changed. Prepare a fresh plan.');if(input.plan.stages.some(s=>s.status!=='pending'))throw new HttpError(400,'Start a fresh background plan; prior job attempts remain recorded.');
 await reconcileProjectJobs(id);const admin=jobsDb();const {data:existing}=await admin.from('studio_build_jobs').select('id,project_id,status').eq('id',input.plan.id).maybeSingle();if(existing){if(existing.project_id!==id)throw new HttpError(409,'Plan identifier already used.');if(['failed','cancelled'].includes(existing.status))throw new HttpError(409,'This attempt has ended. Close this plan and prepare a fresh build.');return Response.json(existing);}
...
export async function DELETE(request:Request,ctx:Context){try{requireSameOrigin(request);const {db,user}=await authorize(request);const {id}=await ctx.params;await requireEditor(db,id,user.id);await reconcileProjectJobs(id);const {error}=await jobsDb().from('studio_build_jobs').update({cancel_requested:true}).eq('project_id',id).in('status',['queued','running']);if(error)throw error;return Response.json({cancelRequested:true});}catch(e){return failure(e)}}
