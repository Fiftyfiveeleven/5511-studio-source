...
export async function POST(request:Request,ctx:{params:Promise<{id:string}>}){
 ...
 try{
  ...
  const artifact=await generateApp(apiKey,prompt,project.name,previous?.files??[],!!project.supabase_url,{requestId,projectId:id,previousSql:previous?.sql??'',images,specification:project.specification,buildId,budgetUsd});
