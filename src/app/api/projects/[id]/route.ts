...
import {specificationSchema,buildPlanSchema} from '@/lib/project-memory';
...
export async function PATCH(request:Request,ctx:Context){try{const {db,user}=await authorize(request);const {id}=await ctx.params;await ownedProject(db,id);await requireEditor(db,id,user.id);const input=z.object({github_connection:githubConnectionSchema.nullable().optional(),specification:specificationSchema.optional(),build_plan:buildPlanSchema.nullable().optional(),name:z.string().trim().min(1).max(80).optional(),supabase_url:z.string().max(200).optional(),supabase_key:z.string().max(2000).optional(),current_revision_id:z.uuid().optional()}).parse(await readBody(request));
