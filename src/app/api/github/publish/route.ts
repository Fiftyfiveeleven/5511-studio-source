...
export async function POST(request:Request){try{
 ...
 z.object({action:z.literal('push'),expectedHead:z.string().regex(/^[a-f0-9]{40}$/).nullable().optional(),projectId:z.uuid(),name:z.string().min(1).max(80),repository:z.string().max(200),branch:z.string().min(1).max(200).regex(/^[a-zA-Z0-9_./-]+$/),files:artifactSchema.shape.files,sql:z.string().max(50000),url:z.string().max(200).nullable(),key:z.string().max(2000).nullable()})]).parse(await readBody(request));
