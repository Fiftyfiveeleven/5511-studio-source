...
export async function POST(request:Request){try{requireSameOrigin(request);const key=credential(request,'openai');if(!key)throw new HttpError(401,'Connect your OpenAI key in Settings.');const input=reviewInputSchema.parse(await readBody(request,3500000));return NextResponse.json(await reviewDesign(key,input));}catch(e){return failure(e)}}
