...
export async function PATCH(request:Request){try{requireSameOrigin(request);const key=await keyFor(request);const limits=z.object({dailyTokens:z.number().int().min(1000).max(2000000),maxOutputTokens:z.number().int().min(1000).max(18000),maxInputCharacters:z.number().int().min(1000).max(150000),dailyBuilds:z.number().int().min(1).max(100)}).parse(await readBody(request));await changeLedger(key,data=>{data.limits=limits});return NextResponse.json({saved:true})}catch(e){return failure(e)}}
