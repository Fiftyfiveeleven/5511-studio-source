...
export async function POST(request:Request){try{
 ...
 const input=z.object({specification:specificationSchema.optional(),buildId:z.uuid().optional(),budgetUsd:z.number().min(0.1).max(100).optional(),images:attachmentsSchema,prompt:z.string().trim().min(5).max(6000),name:z.string().max(80),files:z.array(artifactSchema.shape.files.element).max(80),connected:z.boolean(),requestId:z.uuid(),projectId:z.uuid(),previousSql:z.string().max(50000).default('')}).parse(await readBody(request,3500000));
