...
async function verifyJob(id:string){
 'use step';
const j=await loadJob(id);if(j.cancel_requested){await finishJob(id,'cancelled');return;}try{await jobProgress(id,'Testing navigation and working features');await assertJobAccess(j);const {data:r}=await jobsDb().from('revisions').select('*').eq('id',j.expected_revision).single();if(!r)throw new Error('Saved revision unavailable');const report=isFullstack(r.files)?await checkRuntime(r.files,true):{compiled:true,requirements:[],errors:verifySources(r.files).errors};const passed=report.compiled&&!report.errors.length&&(!isFullstack(r.files)||report.requirements.length>0&&report.requirements.every(x=>x.passed));await finishJob(id,passed?'completed':'failed',passed?undefined:'Acceptance checks failed. Saved checkpoints are available.',report);}catch(e){await finishJob(id,'failed',(e as Error).message);}}
