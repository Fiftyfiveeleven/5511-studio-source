...
export async function estimateGeneration(apiKey:string,prompt:string,name:string,files:SourceFile[],connected:boolean,options:{requestId?:string;projectId?:string;previousSql?:string;previousTurn?:{prompt:string;summary:string};images?:ImageAttachment[];specification?:ProjectSpecification;buildId?:string;budgetUsd?:number}={}){
 ...
 const context=JSON.stringify({project:name,previousTurn:options.previousTurn?{prompt:options.previousTurn.prompt.slice(0,6000),summary:options.previousTurn.summary.slice(0,4000)}:undefined,attachments:images.map(i=>({name:i.name,useAs:i.useAs??'asset'})),sourceIndex:graph?.index,specification:options.specification??{},supabaseConnected:connected,previousFiles:[...route.files].sort((a,b)=>a.path.localeCompare(b.path)),previousSql:route.focused?undefined:options.previousSql??''});
