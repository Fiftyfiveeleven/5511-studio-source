...
export async function estimateGeneration(apiKey:string,prompt:string,name:string,files:SourceFile[],connected:boolean,options:{requestId?:string;projectId?:string;previousSql?:string;images?:ImageAttachment[];specification?:ProjectSpecification;buildId?:string;budgetUsd?:number}={}){
 const images=attachmentsSchema.parse(options.images);
 const route=routeBuild(prompt,files,images.length>0);const graph=isFullstack(files)?dependencyContext(prompt,files):null;if(graph){route.files=graph.files;route.info.contextFiles=graph.files.map(f=>f.path);route.info.omittedCharacters=graph.omitted;route.info.reason=graph.omitted?'Dependency graph selected relevant files and dependents.':'Full runtime context; no safe narrower scope.';}const model=route.model;
 const requestInstructions=(isFullstack(files)?runtimeInstructions:instructions)+(route.focused?' This is a narrowly scoped stylesheet edit. HTML is read-only reference. Return only styles.css when changed and sql=null. Preserve the app name. Do not change markup, behavior or database schema. Other source files are intentionally omitted.':'');
 const limits=(await usageSummary(apiKey)).limits;
 const context=JSON.stringify({project:name,sourceIndex:graph?.index,specification:options.specification??{},supabaseConnected:connected,previousFiles:[...route.files].sort((a,b)=>a.path.localeCompare(b.path)),previousSql:route.focused?undefined:options.previousSql??''});
 ...
 return {route,model,limits,context,requestInstructions,images,reservation,reservedUsd};
}
export async function generateApp(apiKey:string,prompt:string,name:string,files:SourceFile[],connected:boolean,options:{requestId?:string;projectId?:string;previousSql?:string;images?:ImageAttachment[];specification?:ProjectSpecification;buildId?:string;budgetUsd?:number}={}){
 const {route,model,limits,context,requestInstructions,images,reservation,reservedUsd}=await estimateGeneration(apiKey,prompt,name,files,connected,options);
 ...
 const delta=JSON.parse(response.output_text);checkRoutingScope({...delta,files:[...(delta.files??[]),...(delta.patches??[])]},route.editable,route.focused);
 ...
 const artifact=mergeGenerated(delta,files,options.previousSql??'');
