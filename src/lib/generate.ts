import {attachAssets,assetPath,assetInstructions,assetCatalog} from './image-assets';
...
import {z} from 'zod';
export const GENERATOR_VERSION='exact-image-assets-v7';
...
export async function generateApp(apiKey:string,prompt:string,name:string,files:SourceFile[],connected:boolean,options:{requestId?:string;projectId?:string;previousSql?:string;images?:ImageAttachment[];specification?:ProjectSpecification;buildId?:string;budgetUsd?:number}={}){
 ...
 if(images.some(i=>i.useAs!=='reference')&&/logo/i.test(prompt)){
  const latest=assetCatalog(attachAssets([],images));
  if(!latest.some(a=>artifact.files.some(f=>!assetPath(f.path)&&f.content.split(a.symbol).length>2)))throw new HttpError(422,'The requested logo was not connected to the uploaded file. Your previous version is preserved.');
 }
