...
export function exportGithubFiles(input:{projectId:string;name:string;files:SourceFile[];sql:string;url:string|null;key:string|null}){
...
export async function pushGithub(token:string,input:{projectId:string;name:string;repository:string;branch:string;files:SourceFile[];sql:string;url:string|null;key:string|null}){
 ...
 const ref=await github(token,root+refPath);const head=ref.object.sha;const commit=await github(token,root+'/git/commits/'+head);const tree=await github(token,root+'/git/trees/'+commit.tree.sha+'?recursive=1');
 ...
 if(marker){const blob=await github(token,root+'/git/blobs/'+marker.sha);let binding;try{binding=JSON.parse(Buffer.from(blob.content,'base64').toString('utf8'))}catch{throw new HttpError(409,'Repository project marker is invalid.');}if(binding.projectId!==input.projectId)throw new HttpError(409,'This repository belongs to another Studio project. Choose a separate repository.');}
 ...
 const changed=files.filter(f=>{const bytes=Buffer.from(f.content);const sha=createHash('sha1').update(`blob ${bytes.length}\0`).update(bytes).digest('hex');return !tree.tree.some((old:{path:string;sha:string})=>old.path===f.path&&old.sha===sha)});
 if(!changed.length)return {repository:input.repository,branch:input.branch,sha:head,url:`https://github.com/${input.repository}/commit/${head}`,unchanged:true};
 const newTree=await github(token,root+'/git/trees','POST',{base_tree:commit.tree.sha,tree:changed.map(f=>({...f,mode:'100644',type:'blob'}))});
