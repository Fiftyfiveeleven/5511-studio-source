...
export default function BackgroundBuilds({project,api,onUpdate,onActive}:{project:Project;api:(p:string,m?:string,b?:unknown)=>Promise<any>;onUpdate:(p:Project,r:Revision[])=>void;onActive?:(active:boolean)=>void}){
 ...
 useEffect(()=>{setJobs([]);setError('');if(!project.owner_id)return;let stopped=false;let timer:ReturnType<typeof setTimeout>;let version='';async function poll(){try{const d=await api(`/api/projects/${project.id}/jobs`);if(stopped)return;activeCallback.current?.(d.jobs.some(active));setJobs(d.jobs.filter((j:any)=>j.plan.stages.length>0));setError('');const next=d.jobs.map((j:any)=>j.stage+j.status).join();if(next!==version){const fresh=await api('/api/projects/'+project.id);if(!stopped)update.current(fresh.project,fresh.revisions);}version=next;}catch(e){if(!stopped)setError((e as Error).message)}finally{if(!stopped)timer=setTimeout(poll,5000)}}void poll();return()=>{stopped=true;clearTimeout(timer)}},[project.id,project.owner_id]);
