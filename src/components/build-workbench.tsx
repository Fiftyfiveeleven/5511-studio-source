...
export default function BuildWorkbench({project,current,goal,images,api,onProject,onRevision,onClose,onBusy,onComplete,resume=false}:{project:Project;current:Revision|null;goal:string;images:ImageAttachment[];api:(path:string,method?:string,body?:unknown)=>Promise<any>;onProject:(p:Project)=>void;onRevision:(r:Revision)=>void;onClose:()=>void;onBusy:(b:boolean)=>void;onComplete:()=>void;resume?:boolean}){
 ...
 const invalidBudget=!Number.isFinite(plan.budgetUsd)||plan.budgetUsd<0.1||plan.budgetUsd>(quote?.maxBuildUsd??100);
 ...
 <label>Total ceiling for this build, including repair · USD<input type="number" min="0.1" max={quote?.maxBuildUsd??100} step="0.1" disabled={busy} value={plan.budgetUsd} onChange={e=>setPlan(p=>({...p,budgetUsd:Number(e.target.value)}))}/></label>{!cloudBuild&&<label className="repair-option"><input type="checkbox" disabled={busy||plan.repairUsed} checked={plan.allowRepair} onChange={e=>setPlan(p=>({...p,allowRepair:e.target.checked}))}/>Allow one paid repair within this ceiling</label>}<p className="settings-help">{cloudBuild?'Your build saves each completed stage. You can leave this page and return later. Progress appears in the project; building stops if an error occurs. Hosting compute charges are separate from the AI ceiling.':'Keep this page open while building. Each completed stage is saved. Pausing takes effect after the current request.'}</p>
