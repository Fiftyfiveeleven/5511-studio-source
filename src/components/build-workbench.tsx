...
export default function BuildWorkbench({project,current,goal,images,api,onProject,onRevision,onClose,onBusy,onComplete,resume=false}:{project:Project;current:Revision|null;goal:string;images:ImageAttachment[];api:(path:string,method?:string,body?:unknown)=>Promise<any>;onProject:(p:Project)=>void;onRevision:(r:Revision)=>void;onClose:()=>void;onBusy:(b:boolean)=>void;onComplete:()=>void;resume?:boolean}){
 ...
 async function queue(){if(running.current)return;running.current=true;setBusy(true);setError('');try{await api(`/api/projects/${project.id}/jobs`,'POST',{plan,images,runtime});onClose();}catch(e){setError((e as Error).message)}finally{running.current=false;setBusy(false)}}
 ...
 const cloudBuild=!!project.owner_id&&runtime==='nextjs';
 const invalidBudget=false;
 const started=plan.stages.some(s=>s.status!=='pending');
 return <div className="modal-backdrop"><section className="modal build-workbench" role="dialog" aria-modal="true" aria-label="Build plan"><h2>Build plan</h2><p>{plan.goal}</p>{quote?<p className="settings-notice">Next request: <b>{quote.model}</b> · estimated allowance <b>${quote.reservedUsd.toFixed(3)}</b> · Usage tracking is on; spending limits are off.</p>:<p>Checking model…</p>}
 ...
 {!cloudBuild&&<label className="repair-option"><input type="checkbox" disabled={busy||plan.repairUsed} checked={plan.allowRepair} onChange={e=>setPlan(p=>({...p,allowRepair:e.target.checked}))}/>Allow one paid repair</label>}<p className="settings-help">{cloudBuild?'Your build saves each completed stage. You can leave this page and return later. Progress appears in the project; building stops if an error occurs. Hosting compute is billed separately.':'Keep this page open while building. Each completed stage is saved. Pausing takes effect after the current request.'}</p>
 ...
 {!busy&&plan.stages.some(s=>s.status==='failed')&&<button className="subtle-button" onClick={()=>{setPlan(p=>({...p,status:'paused',stages:p.stages.map(s=>s.status==='failed'?{...s,id:crypto.randomUUID(),status:'pending'}:s)}));setError('A new paid attempt is prepared. Earlier usage remains in your history. Click Resume to send it.')}}>Prepare a new attempt for failed stages</button>}
 <div className="settings-actions">{busy?(cloudBuild?<button className="primary-button" disabled>Starting build…</button>:<button className="subtle-button" onClick={()=>{pause.current=true}}>Pause after this request</button>):<button className="primary-button" disabled={!quote||invalidBudget||plan.status==='complete'} onClick={()=>void (cloudBuild?queue():run())}>{plan.status==='ready'?'Build':'Continue build'}</button>}<button className="subtle-button" disabled={busy} onClick={async()=>{try{await save(plan);onClose()}catch(e){setError((e as Error).message)}}}>Save plan for later</button><button className="subtle-button" disabled={busy} onClick={onClose}>Close</button></div>{checking&&<p>Running isolated browser checks…</p>}{plan.status==='complete'&&<p role="status">Build stages saved. Browser smoke checks passed; review any warnings before publishing.</p>}</section></div>;
}
